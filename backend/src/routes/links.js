...
module.exports = function createLinksRouter(broadcast) {
  ...
  router.post('/', async (req, res) => {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        ...
        const link = formatLink(result.rows[0], req);
        broadcast({ type: 'link:created', data: link });
  ...
  router.put('/:id', async (req, res) => {
    ...
    try {
      ...
      const link = formatLink(result.rows[0], req);
      broadcast({ type: 'link:updated', data: link });
      res.json(link);
    } catch (err) {
  ...
  router.delete('/:id', async (req, res) => {
    ...
    try {
      ...
      broadcast({ type: 'link:deleted', data: { id: parseInt(id) } });
